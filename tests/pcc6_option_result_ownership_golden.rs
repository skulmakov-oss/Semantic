...
fn positive_option_result_ownership_e2e_golden() {
    let src = include_str!("fixtures/pcc6_option_result_diagnostics/positive_option_result_ownership.sm");
    ...
    for envelope in envelopes {
        for path in envelope.borrowed_paths.iter().chain(envelope.write_paths.iter()) {
    ...
    assert!(adt_payload_count >= 3, "Expected at least 3 AdtPayload components, found {}", adt_payload_count);
