...
- **prove**: Run the verifier over the subject to guarantee semantic soundness, type safety, and memory properties without generating execution side effects.
...
- **seal**: Finalize the current state or artifact of the subject, creating an immutable, cryptographically verifiable boundary (often yielding a `.smc` or token).
