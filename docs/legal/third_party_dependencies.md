...
### 4.2 Retired Workbench TS/Tauri dependencies (historical)

> [!NOTE]
> All dependencies previously associated with `apps/workbench_ts_tauri_legacy` (`src-tauri/Cargo.toml` and `package.json`) were retired under Issue **#1859** to eliminate unmaintained npm and Tauri dependency surfaces from `main`. See [`docs/history/workbench_ts_tauri_legacy.md`](../history/workbench_ts_tauri_legacy.md) for historical provenance.
