...
| `match`, guards, patterns | Included stable candidate | `tests/match_surface_qualification.rs`, PCC5/PCC6 acceptance | exhaustiveness, guard type, pattern-family and arm-type negatives, plus compile-phase lowering-rejection fixtures for scalar (`quad`/`i32`/`u32`) and wildcard-present/absent sum-family or-patterns, genuine multi-value and oversized-singleton `i32` range arms, and a check-phase suffixed-integer-literal range-bound rejection fixture |
