...
| `match`, guards, patterns | Included stable candidate | `tests/match_surface_qualification.rs`, PCC5/PCC6 acceptance | exhaustiveness, guard type, pattern-family and arm-type negatives, plus compile-phase lowering-rejection fixtures for scalar (`quad`/`i32`/`u32`) or-patterns and wildcard-present/absent or-patterns across all three sum families (enum, `Option(T)`, `Result(T, E)`), genuine multi-value and oversized-singleton `i32` range arms, a check-phase suffixed-integer-literal range-bound rejection fixture, and runtime-defect fixtures pinning the `u32` literal/range match trap and the exclusive `5..5` singleton-range miscompilation |
