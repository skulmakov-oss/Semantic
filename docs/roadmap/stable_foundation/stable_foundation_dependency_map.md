...
| SSF-08 / #1579 | Selected abstraction surface | Ownership Position A/B, value paths, frames, host ownership, quotas | Positive/negative ownership and deterministic failure suite | Completed |
| SSF-09 / #1580 | Canonical diagnostics and project symbols | Diagnostic schema, formatter, LSP/editor baseline | CLI/LSP parity, idempotence, protocol fixtures | **Active** |
...
- **SSF-08 → SSF-09** (transition base `bb4beec123e729c41e244ab204a66bf9cee81344`): SSF-08's ownership/memory positioning contract is fully qualified - Lane 5's resource-quota/failure-taxonomy residual (#1759, #1760, #1761, #1762, #1763, #1900, #1902) closed, and a final acceptance-reconciliation audit (PR #1916) freshly re-derived all seven of #1579's acceptance criteria as SATISFIED with zero scoped blockers. #1579 closed 2026-09-11 (`stateReason: COMPLETED`). Full evidence: `ssf08_closure_audit.md`. This entry is itself the governance-only transition that makes SSF-09's activation real - `.harness/current.task.yaml` and this map are the two independent "which phase is active" authorities `tests/ssf_status_drift.rs::ssf_active_phase_matches_across_dependency_map_and_harness` checks agree; no SSF-09 implementation, diagnostic carrier, schema, or LSP code is authorized by this entry. A separate Diagnostic Authority decision (source surface / diagnostic identity / file identity / source range) is required before any individual diagnostic/span finding is repaired, mirroring SSF-08's own Position A/B precedent.
