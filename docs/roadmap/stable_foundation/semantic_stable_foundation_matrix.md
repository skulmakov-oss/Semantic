...
| measured numeric forms | F/I | **Experimental** | Narrow carriers/type semantics exist; the excluded-arithmetic boundary is pinned per-operator in `crates/sm-front/src/typecheck.rs`: fx binary and unary `+`/`-` (`measured_fx_addition_still_reports_narrow_slice_gap`, `measured_fx_subtraction_reports_narrow_slice_gap`, `measured_fx_unary_minus_reports_narrow_slice_gap`), `*`/`/`/`%` (`measured_arithmetic_rejects_mul`, `_div`, `_mod`), mismatched units (`measured_arithmetic_rejects_mismatched_units`), and unary and binary `+`/`-` on `i32`/`u32` (`measured_i32_unary_minus_rejects`, `measured_u32_unary_minus_rejects`, `measured_i32_unary_plus_rejects`, `measured_u32_unary_plus_rejects`, `measured_i32_binary_addition_rejects`, `measured_i32_binary_subtraction_rejects`, `measured_u32_binary_addition_rejects`, `measured_u32_binary_subtraction_rejects`; the `u32` cases use `u32`-suffixed literals so the measured-lift actually reaches the operator check). Same-unit `f64` binary and unary `+` typecheck (`measured_f64_addition_typechecks`, `measured_f64_unary_minus_typechecks`). `docs/spec/types.md` and `docs/spec/source_semantics.md` restrict their binary/unary `+`/`-` rules to measured `f64` accordingly; `==`/`!=` remain valid across all measured base types per `supports_stable_equality_type_inner`. | Boundary pinned in SSF-07; widening remains a later decision. |
