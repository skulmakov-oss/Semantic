...
| `std.math` | F/I/V/R/D | **Qualified limited release (`sqrt`/`abs`); Roadmap (`sin`/`cos`/`tan`/`pow`)** | SSF-07 promotes `sqrt`/`abs` to Selected. `abs` is bit-exact for every input. `sqrt` is IEEE-754 correctly-rounded and cross-platform bit-exact for `x >= 0.0` only; for `x < 0.0` it returns `NaN`, but the exact `NaN` bit pattern is explicitly excluded from the frozen contract (IEEE 754 doesn't mandate one, and it can legitimately differ across platforms) — only the semantic "is NaN" property (`sqrt(x) != sqrt(x)`) is promised. `sin`/`cos`/`tan`/`pow` remain experimental pending transcendental determinism policy. | `sqrt`/`abs` closed in SSF-07 within this domain qualifier; transcendental subset routed to SSF-07/10. |
