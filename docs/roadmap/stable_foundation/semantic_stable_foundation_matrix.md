...
| First-wave generics / monomorphisation | F/I/V/R | **Landed but unqualified** | Frontend generic function declaration admission and call-site type inference/substitution are landed and qualified by focused executable evidence (#1634/#1648/#1649); generic record/ADT declarations are narrowed to zero type parameters and qualified the same way (#1650). No IR monomorphisation or specialization pass exists anywhere in `crates/sm-ir`: prior to SSF-07 #1717 this meant a generic function's `type_params` had no admission check at the IR boundary at all, so a declared-but-unreferenced type parameter (e.g. `fn marker<T>(x: i32) -> i32`) silently lowered as an ordinary, non-generic `IrFunction` with its metadata discarded. #1717 closes that gap with a deterministic, evidenced rejection at the shared IR lowering boundary — every generic function declaration now fails fast with a diagnostic naming concrete IR monomorphisation as not implemented, rather than executing with type parameters silently erased. Monomorphisation/execution itself remains unimplemented; V/R are never reached for a generic function, since I now rejects first. | Exact subset in SSF-01/07; IR monomorphisation itself remains roadmap, not scheduled by SSF-07. |
