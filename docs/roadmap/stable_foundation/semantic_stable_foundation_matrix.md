...
| OWN0 tuple/direct-record paths | V/R | **Landed but unqualified** | Frozen tuple+record scope; the D7/golden-covered scenarios (sibling/same-path/parent-child/child-parent write, multi-frame cleanup) are qualified and pass. General-case qualification is blocked: #1709 is a false-negative-direction bug inside this exact frozen contract (nested `if`/`loop`-expression lowering can drop real tuple/record Borrow events); #1725 (root-identity) and #1726 (event-timing) affect every path kind, including this one, and are the normative Path-identity/Event-timing invariants this record itself states are currently violated. Corrected 2026-09-02: the prior "Landed and qualified on `main`" status was a maturity claim the decision record's own evidence contradicted, not just a caveat-worthy detail. | Position A selected in SSF-08 (`docs/roadmap/stable_foundation/ssf08_ownership_position_decision.md`); close #1709/#1725/#1726 before this family reads as qualified beyond its tested scenarios. |
