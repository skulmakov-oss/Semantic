...
| `std.text` | F/I/V/R/D | **Qualified limited release** | Exact UTF-8 text equality/concat and bounded scalar `to_text`; no indexing/normalization. SSF-07 froze the operator boundary against every non-text operand family (`i32`, `u32`, `bool`, `quad`, `Map`, `Record` all rejected with the same "text + text operands" message; `Sequence` rejected earlier by the ordered-sequence operator gate with a distinct message) and every `to_text`-unadmitted input, consolidated in `tests/ssf07_text_family_freeze.rs`. | Operator/`to_text` boundary closed in SSF-07; `std.seq`/`std.map` and the sequence-indexing scope question remain open for a follow-up. |
