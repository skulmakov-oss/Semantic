...
| Immutable first-wave closures | F/I/V/R | **Landed and qualified on `main`** | Short-lambda positive/negative qualification. The captureful exclusion is explicit, via a dedicated "capture-free only" diagnostic (`rustlike_parser_rejects_captureful_short_lambda`). The multi-arg exclusion is deterministic but incidental, not explicit: the grammar has no multi-parameter lambda production at all, so `(x, y) => ...` falls through to ordinary tuple-expression parsing and fails with an unrelated generic parser error, and `Closure(f64, f64 -> f64)` fails because `parse_type()` only ever parses one param type — both now pinned as regression tests (`rustlike_parser_rejects_multi_arg_short_lambda`, `rustlike_parser_rejects_multi_arg_closure_type_annotation`) in SSF-07, matching the more accurate "not part of the current contract" / "Explicit Non-Goals" wording already used in `docs/spec/source_semantics.md` and `first_class_closures_full_scope.md`. | Bound in SSF-01/07. |
...
| First-wave generics: frontend declaration admission and call-site inference | F | **Landed and qualified on `main`** | Generic function declaration admission (`build_fn_table`, at most one type parameter per definition site) and call-site type inference/substitution (`collect_generic_constraints`/`subst_apply`) are landed and qualified by focused executable evidence (#1634/#1648/#1649). Generic record/ADT declarations are narrowed to zero type parameters and qualified the same way (#1650) — no applied nominal type representation or construction path exists for them. | Exact subset in SSF-01/07. |
| First-wave generics: IR monomorphisation / execution | I/V/R | **Roadmap** | No IR monomorphisation or specialization pass exists anywhere in `crates/sm-ir` (only `StructuralCleanup`/`CrystalFold`). Prior to SSF-07 #1717, a generic function's `type_params` had no admission check at the IR boundary at all, so a declared-but-unreferenced type parameter (e.g. `fn marker<T>(x: i32) -> i32`) silently lowered as an ordinary, non-generic `IrFunction` with its metadata discarded — a false-ready surface, not partial execution support. #1717 closes that gap with a deterministic, evidenced rejection at the shared IR lowering boundary: every generic function declaration now fails fast with a diagnostic naming concrete IR monomorphisation as not implemented, rather than executing with type parameters silently erased. V/R are never reached for a generic function, since I now rejects first. | IR monomorphisation itself remains roadmap, not scheduled by SSF-07; #1717 closes only the false-ready admission gap, it does not implement execution. |
...
| Broader static traits/protocols and impl coherence | F/I/V/R | **Landed but unqualified** | Trait/impl parsing, duplicate/coherence, and conformance evidence exists beyond the admitted direct-record slice, split across two validators in `crates/sm-front/src/typecheck.rs`: `validate_trait_coherence` (called first) rejects blanket/generic impls (non-empty `type_params`) and duplicate impls of the same (trait, type) pair; `validate_impl_conformance` (called second) is now frozen bidirectionally in SSF-07 — every trait-declared method must be present with matching arity/params/return (pre-existing), and every impl method must be declared by the trait, no fewer no more (`impl_extra_method_not_in_trait_is_rejected`, new in SSF-07). Also newly pinned in `validate_impl_conformance`: `impl_duplicate_method_name_within_impl_is_rejected`, `impl_of_unknown_trait_is_rejected` (previously-untested existing branches). Explicit non-goal: impl methods have no invocation syntax outside the hardcoded Iterable `next()` for-loop desugaring — general trait method dispatch/UFCS wiring remains deferred beyond SSF-07. | Conformance direction closed in SSF-07; dispatch/UFCS and generics/monomorphisation remain open. |
...
| measured numeric forms | F/I | **Experimental** | Narrow carriers/type semantics exist; `crates/sm-front/src/typecheck.rs` pins the full binary/unary `+`/`-` matrix across all four measured base types (all cases pass against unmodified code). `f64` (accepted, matching units): `measured_f64_addition_typechecks`, `measured_f64_binary_subtraction_typechecks`, `measured_f64_unary_plus_typechecks`, `measured_f64_unary_minus_typechecks`. `fx` (narrow-slice-gap rejection): `measured_fx_addition_still_reports_narrow_slice_gap`, `measured_fx_subtraction_reports_narrow_slice_gap`, `measured_fx_unary_plus_reports_narrow_slice_gap`, `measured_fx_unary_minus_reports_narrow_slice_gap`. `i32` (unsupported-operator rejection): `measured_i32_binary_addition_rejects`, `measured_i32_binary_subtraction_rejects`, `measured_i32_unary_plus_rejects`, `measured_i32_unary_minus_rejects`. `u32` (unsupported-operator rejection, `u32`-suffixed literals so the measured-lift reaches the operator check): `measured_u32_binary_addition_rejects`, `measured_u32_binary_subtraction_rejects`, `measured_u32_unary_plus_rejects`, `measured_u32_unary_minus_rejects`. `*`/`/`/`%` are tested against measured `f64` (`measured_arithmetic_rejects_mul`, `_div`, `_mod`) and are family-agnostic by construction, not by separate per-family tests: unlike `+`/`-` (which branch on `base` with `if *base == Type::F64`/`if *base == Type::Fx` guards, so behavior genuinely differs per family and each family needs its own test), the `BinaryOp::Mul | BinaryOp::Div | BinaryOp::Mod` arm in `typecheck.rs` carries no such guard and rejects unconditionally once either operand is any measured type -- so an `i32`/`u32`/`fx` case would exercise the identical line, not a different one. Mismatched units are rejected regardless of operator (`measured_arithmetic_rejects_mismatched_units`). `docs/spec/types.md` and `docs/spec/source_semantics.md` restrict their binary/unary `+`/`-` rules to measured `f64` accordingly; `==`/`!=` remain valid across all measured base types per `supports_stable_equality_type_inner`. | Boundary pinned in SSF-07; widening remains a later decision. |
...
| `std.math` (`sqrt`, `abs`) | F/I/V/R/D | **Landed and qualified on `main`** | SSF-07 promotes `sqrt`/`abs` to Selected via focused test evidence, not a Gate 1 admission. `abs` is bit-exact for every input. `sqrt` is IEEE-754 correctly-rounded and cross-platform bit-exact for `x >= 0.0` only; for every other admitted input (`x < 0.0`, or `x` already `NaN`) it returns `NaN`, but the exact `NaN` bit pattern is explicitly excluded from the frozen contract (IEEE 754 doesn't mandate one, and it can legitimately differ across platforms) — only the semantic "is NaN" property (`sqrt(x) != sqrt(x)`) is promised. `sqrt`/`abs` are reserved function names (see `docs/spec/foundation_stdlib_v0.md`). | Closed in SSF-07 within this domain qualifier. |
| `std.math` (`sin`, `cos`, `tan`, `pow`) | F/I/V/R/D | **Roadmap** | Remain experimental pending transcendental determinism policy; not promoted by this change. | Transcendental subset routed to SSF-07/10. |
| `std.text` | F/I/V/R/D | **Qualified limited release** | Exact UTF-8 text equality/concat and bounded scalar `to_text`; no indexing/normalization. SSF-07 froze the operator boundary against representative non-text operand families (`i32`, `u32`, `bool`, `quad`, `Map`, `Record` all rejected with the same "text + text operands" message; `Sequence` rejected earlier by the ordered-sequence operator gate with a distinct message) — the remaining admitted families (`f64`, `fx`, `unit`, tuples, `Option`, `Result`) share the same unconditional rejection branch, confirmed empirically rather than assumed — and every `to_text`-unadmitted input (record, map, sequence, wrong arity), consolidated in `tests/ssf07_text_family_freeze.rs`. | Operator/`to_text` boundary closed in SSF-07; `std.seq`/`std.map` and the sequence-indexing scope question remain open for a follow-up. |
...
| Quotas/fuel and trap taxonomy | V/R | **Landed but unqualified** | `Frames`/`StackDepth`/`Registers`/`EffectCalls` quotas are enforced and tested; `Steps`/`Calls`/`ConstPool`/`TraceEntries` are defined but have zero enforcement call sites (#1759-#1761); `ExecutionConfig.context` can disagree with the `quotas` actually enforced (#1762); most `RuntimeTrap` variants are never constructed, with a parallel `RuntimeError` vocabulary used instead (#1763). Corrected 2026-09-02: the prior "Landed and qualified on `main`" status contradicted this same evidence, gathered by the SSF-08 decision record itself (`docs/roadmap/stable_foundation/ssf08_ownership_position_decision.md`). | Close #1759-#1763 in SSF-08 before this family reads as qualified; VM/spec tests alone do not cover the full claimed vocabulary. |
| OWN0 tuple/direct-record paths | V/R | **Landed but unqualified** | Frozen tuple+record scope; the D7/golden-covered scenarios (sibling/same-path/parent-child/child-parent write, multi-frame cleanup) are qualified and pass. General-case qualification is blocked: #1709 is a false-negative-direction bug inside this exact frozen contract (nested `if`/`loop`-expression lowering can drop real tuple/record Borrow events); #1724 (lexical binding identity), #1725 (OWN0 root-identity), and #1726 (event-timing) affect every path kind, including this one, and #1725/#1726 are the normative Path-identity/Event-timing invariants this record itself states are currently violated. Corrected 2026-09-02: the prior "Landed and qualified on `main`" status was a maturity claim the decision record's own evidence contradicted, not just a caveat-worthy detail. | Position A selected in SSF-08 (`docs/roadmap/stable_foundation/ssf08_ownership_position_decision.md`); close #1709/#1724/#1725/#1726 before this family reads as qualified beyond its tested scenarios. |
...
| Human-readable diagnostics and source spans | F/I/V/C | **Landed but unqualified** | Human-readable diagnostics exist and are usable end to end via `smc check`/`smc lint`, backed by PCC negative suites and CLI diagnostic parity tests. Source-location preservation is only partially qualified: the Logos-DSL semantic layer preserves a real position end to end, but the RustLike-profile parse/type/import paths can discard an already-computed real position in favor of `SourceMark::default()` (#1698, #1699). Imported-module warnings can separately lose authoritative module identity during aggregation (#1697). No single span/file-identity authority exists (four incompatible position models coexist and do not interconvert; `file_id` is always `0`); no machine-readable schema exists at all. Corrected 2026-09-11: the prior "Landed and qualified on `main`" status was contradicted by this same evidence, gathered by the SSF-09 entry reconnaissance ahead of its own Diagnostic Authority decision. | Diagnostic Authority decision, then the repair DAG (#1670 -> #1697/#1698/#1699 -> catalog coherence -> canonical carrier -> versioned schema) in SSF-09, before this family reads as qualified. |
