...
| `std.math` (`sqrt`, `abs`) | `tests/pcc8_stdlib_acceptance.rs` (`positive_math_basic.sm`, including the `sqrt(x) != sqrt(x)` NaN pin for both `x < 0.0` and a nested already-`NaN` input) | selected surface is `f64 -> f64` only; type/arity mismatches reject via the existing generic builtin-call diagnostics; `sqrt`/`abs` name collision with a user-defined function is rejected deterministically at typecheck time (`crates/sm-front/src/lib.rs`'s `build_fn_table_rejects_math_builtin_name_collision`), not silently shadowed at runtime | `sqrt`/`abs` call straight into Rust `f64::sqrt`/`f64::abs`, unmetered and host-effect-free, per `crates/sm-vm/src/semcode_vm.rs`; bit-exact baseline is scoped to `sqrt(x)` for `x >= 0.0` and all of `abs` — `sqrt`'s `NaN` bit pattern for `x < 0.0` or an already-`NaN` input is explicitly out of scope |
| `std.text` | `tests/pcc3_text_core_gate.rs`, `tests/pcc8_stdlib_acceptance.rs` | `tests/pcc_stdlib_negative.rs`, `tests/pcc8_stdlib_diagnostics.rs` | PCC3 lowering stability and CTF stdlib traces; SSF-07 compatibility freeze consolidated in `tests/ssf07_text_family_freeze.rs` (representative non-text `+` operand families covering both the text-specific and sequence-gate rejection paths — remaining families share the same unconditional branch, confirmed empirically — plus `to_text`'s full negative surface: record, map, sequence, wrong arity) |
