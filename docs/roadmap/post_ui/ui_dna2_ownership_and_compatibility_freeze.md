...
| O07 | Binding Graph | `crates/prom-ui::binding_graph` | target module | LANDED foundation through #1491; observation/dirty v0 contract frozen through #1513; crate-private engine implementation and qualification landed through #1514; crate-private caller-supplied Semantic observation adapter v0 implementation and qualification landed through #1515; live Semantic reads and runtime integration remain absent | read-side relationships between structure, roles, semantic refs, action surfaces, evidence anchors | patch stream, denial/recovery, task/control semantics, runtime effects, admission | may adapt slot-intent carrier evidence |
...
| O15 | Denial/recovery projection | `crates/prom-ui::denial_recovery` | target module | Denial/recovery v0 normative contract, crate-private implementation, inert patch composition and executable qualification carried by this change; admission execution, recovery authority, patch application and runtime integration remain absent | denial projection, recovery outlets, local/session denial distinction, acknowledgement/retry/resume/cancel-suffix presentation contracts | admission policy, invented recovery behavior, Semantic truth | consumes caller-supplied outcomes and opaque references; constructs inert patches only |
| O16 | Task projection | `crates/prom-ui::task_projection` | target module | Task projection v0 normative contract, crate-private implementation, inert patch composition and executable qualification carried by this change; task execution, task truth, patch application and runtime integration remain absent | `TaskRecord` projection, progress presentation, task control projection, task-scope visibility | task truth, task engine, capability policy | consumes caller-supplied outcomes and opaque references; constructs inert patches only |
| O17 | Connectivity/freshness projection | `crates/prom-ui::connectivity_projection` | target module | Freshness projection v0 normative contract, crate-private implementation and executable qualification carried by this change; live connectivity reads, control authority, patch application and runtime integration remain absent | Fresh/stale/offline projection, observation-only and cache-only presentation, `PendingUnknown`, `Resyncing` projection | connection truth, control authority, offline critical action queue | consumes caller-supplied freshness and emits inert patch operations |
...
| N08 | MODULE | `crates/prom-ui::binding_graph` | read-side binding relationship graph | landed foundation; observation/dirty v0 contract frozen; crate-private engine implementation and qualification carried by this change; Semantic adapters and runtime integration absent |
