...
| Malformed-field rejection tests | future reader/parser fixtures + basis guard | missing | General reader/parser behavior remains blocked until malformed cases exist. |
| Unknown-field policy tests | future reader/parser fixtures + basis guard | missing | Unknown-field handling is still unspecified for a general claim. |
| Duplicate-field policy tests | future reader/parser fixtures + basis guard | missing | Duplicate-field handling is still unspecified for a general claim. |
| Field-ordering policy tests | future reader/parser fixtures + basis guard | missing | Field-ordering handling is still unspecified for a general claim. |
